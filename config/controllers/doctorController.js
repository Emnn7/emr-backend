...
exports.getMyPatients = catchAsync(async (req, res, next) => {
  ...
  const primaryPatients = await Patient.find({ primaryDoctor: req.user.id })
    .select('firstName lastName phone dateOfBirth gender')
    .lean();

  // Get patients from appointments
  const appointmentPatients = await Appointment.find({ doctor: req.user.id })
    .distinct('patient');
  
  const appointmentPatientsData = await Patient.find({ 
    _id: { $in: appointmentPatients },
    primaryDoctor: { $ne: req.user.id } // Exclude already included primary patients
...
exports.getMyLabOrders = catchAsync(async (req, res, next) => {
  const labOrders = await LabOrder.find({ doctor: req.user.id })
    .populate('patient', 'firstName lastName phone')
    .sort('-createdAt');
...
exports.getPatientMedicalHistory = async (req, res) => {
  try {
    const isAssigned = await verifyDoctorPatient(req.user.id, req.params.patientId);
    if (!isAssigned) {
