...
exports.getAllLabReports = catchAsync(async (req, res, next) => {
  ...
  if (req.user.role === ROLES.DOCTOR) {
    ...
    query = LabReport.find({ performedBy: req.user.id });
  } else {
...
exports.getLabReport = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    labReport.labOrder.doctor._id.toString() !== req.user.id.toString()
  ) {
    return next(
...
exports.updateLabReport = catchAsync(async (req, res, next) => {
  ...
  if (
    req.user.role !== ROLES.ADMIN &&
    labReport.performedBy.toString() !== req.user.id.toString()
  ) {
    return next(
...
exports.verifyLabReport = catchAsync(async (req, res, next) => {
  ...
  labReport.verifiedBy = req.user.id;
  ...
  await AuditLog.create({
    ...
    user: req.user.id,
    userModel: capitalizeFirstLetter(req.user.role),
    changes: { status: 'verified', verifiedBy: req.user.id },
    ipAddress: req.ip,
    userAgent: req.get('User-Agent')
  });
