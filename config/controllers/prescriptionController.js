...
exports.getAllPrescriptions = catchAsync(async (req, res, next) => {
  ...
  if (req.user.role === ROLES.DOCTOR) {
    query = Prescription.find({ doctor: req.user.id });
  } else {
...
exports.getPrescription = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    prescription.doctor._id.toString() !== req.user.id.toString() &&
    prescription.patient._id.toString() !== req.user.id.toString()
  ) {
    return next(
...
exports.updatePrescription = catchAsync(async (req, res, next) => {
  ...
  if (
    req.user.role !== ROLES.ADMIN &&
    prescription.doctor.toString() !== req.user.id.toString()
  ) {
    return next(
