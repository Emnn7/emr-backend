...
exports.register = catchAsync(async (req, res, next) => {
  ...
  await AuditLog.create({
    ...
    user: isInitialSetup ? newUser._id : req.user.id, // Log admin who created the user
...
exports.getMe = catchAsync(async (req, res, next) => {
  const user = await req.user.constructor.findById(req.user.id);
  res.status(200).json({
    status: 'success',
    data: { user }
  });
...
exports.deactivateMe = catchAsync(async (req, res, next) => {
  await req.user.constructor.findByIdAndUpdate(req.user.id, { active: false });
